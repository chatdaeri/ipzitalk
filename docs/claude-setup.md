...
In the CLI, run `/reload-plugins`, open `/mcp`, and complete OAuth for the plugin-provided `ipzitalk` server. In the Desktop Code tab, confirm the plugin in `+` → `Plugins`, open a new local session, and complete any OAuth prompt before a representative Remote call. Remote includes 27 Skills (main 6 + sub 21) and the hosted data path.
