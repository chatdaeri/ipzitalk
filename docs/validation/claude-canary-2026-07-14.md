...
The follow-up correction used one File Edit and no MCP, Bash, shell, or web call. The transcript shows an exact one-line replacement of `window.__DATA__.mixNote` with the short disclosed-unit basis sentence; no other field or fixed template content changed. The final file contains no internal provenance terms or absolute path and retains the byte-identical fixed template. Mark `ipzitalk-presale-report` as the first of five packaged Skills with a strict final artifact pass. Keep the single-pass behavior improvement open because the initial render required a corrective turn.
