...
## Remote Skill runtime follow-ups 0.1.4

Codex Desktop representative runs completed for all five packaged Skills. The follow-up source work started from checkpoint `7fe42c1`; RED contract `8574f38` reproduced three runtime problems, and GREEN `0bfa0cb73a00958dd2825ee00150166f6a95d055` fixed them without changing templates or shared renderers.
...
The source test suite passed 22/22 and the namespace validator passed for 24 MCP-dependent Skills. The generic `quick_validate.py` could not start because PyYAML is absent; no dependency installation was attempted. The package lock advanced to the local-only source commit above, source/target directory and artifact diffs were empty, and `node scripts/validate-package.mjs` passed. Remote manifests and the Claude marketplace advanced to `0.1.4`. User-profile reinstall and the two targeted Desktop reruns remain pending; no push or pull request was created.
