...
Claude Desktop의 기존 세션은 새 플러그인을 다시 읽지 않는다. 새 로컬 Code 탭 세션을 열고 `+` → `Plugins` → `ipzitalk-remote` → `ipzitalk-recent-market-trend`를 선택한다.

확인 항목:

1. 목적 없는 요청에서 네이티브 목적 선택 UI가 먼저 나타나는가
2. 목적 선택 전 Remote MCP 데이터 호출이 0회인가
3. 목적 선택 후 리포트가 완성되는가
4. 상세 표에서 지역 열은 왼쪽, 이후 헤더·숫자 값은 오른쪽으로 맞는가
5. 다음 행동 제안이 `시세 추이 분석`이 아니라 `실거래 추이 분석`으로 표시되는가
6. `auditIncomplete:false`와 `2N+R` 호출식이 일치하는가
