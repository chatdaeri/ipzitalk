...
| `get_complex_info(kapt_code, detail=false)` | 사용자가 지정한 사용승인일 결측 코드의 선택적 실시간 보강 |
...
3. **K-apt 매칭** — 외부 후보 배열을 enrich 1회로 처리하고 `matched`만 확정 분석 대상으로 사용한다. `ambiguous`·`not_found`는 후보 좌표·주소와 사유만 표시하고 자동확정하지 않는다.
   - `matched` 좌표는 그대로 재사용하고 추가 지오코딩하지 않는다. 좌표가 `null`인 확정 항목만 별도 지오코딩으로 해소한다.
   - `use_approval_date=null`은 결측으로 표시하고 밴드·순위에서 제외한다.
   - `get_complex_info_batch(detail=false|true)`는 같은 REF_DB 기본행을 읽거나 상세 API만 추가하므로 사용승인일을 보강하지 않는다.
   - 명시적으로 사용자가 특정 결측 코드의 최신값을 요청한 경우에만 `get_complex_info(kapt_code, detail=false)`를 코드별로 호출한다. `N`개면 추가 호출 수를 `+N`으로 기록한다.
...
기본 MCP 호출 예산은 기준점 해소 1회 + 후보 검색 1회 + enrich 1회 + 지도 1회 = 총 4회다. 명시적으로 선택한 사용승인일 실시간 보강만 `+N`이다.
...
| version | 날짜 | 변경 |
|---|---|---|
| 1.0.0 | 2026-07-09 | 패키지 배포본. 사용승인일 결측 밴드/순위 제외+사유 카드, 표본 수·원천 후보 수 병기, 디자인 규칙 인라인, 지도 TTL·눈금마커·`.map` overflow 규칙 반영 |
...
| 1.0.2 | 2026-07-10 | **지도를 분포와 순위 사이 전폭 카드로 이동.** 기존엔 `.g2` 우측 0.9fr 칸에 있어 `min-width:720px` iframe이 가로 스크롤로 밀려 지도가 잘려 보였다. 레이아웃을 분포 → 지도 → 순위 세로 스택으로 변경(형제 랭킹 스킬과 동일). frontmatter `version` 이 변경 이력과 어긋나 있던 것도 함께 정정 |
| 1.0.3 | 2026-07-10 | **푸터 4단 분리.** 표본 제외 목록을 `.exbox` 박스로 독립시켜 필수 단서(hedges)와 시각적으로 구분. 기존엔 `<br>` 한 흐름이라 제외 13줄에 단서가 파묻혔다. 출처는 `.srcline` 구분선으로 분리 |
| 1.0.4 | 2026-08-12 | 외부 후보는 enrich `matched`만 확정하고 과거 이름 단독 복구를 폐기. 사용승인일 REF_DB 결측은 info batch로 보강하지 않으며 명시적으로 지정된 코드만 단건 기본 API로 `+N` 보강 |
