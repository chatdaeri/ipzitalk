...
| 버전 | 날짜 | 내용 |
|---|---|---|
| 1.2.7 | 2026-07-27 | review 매핑은 이름 단독 우회를 금지하고 exact 법정동·전체 지번·K-apt 공식명 제한 조회만 허용 |
| 1.2.6 | 2026-07-27 | `get_complex_info`의 `kapt_code`를 매매 실거래 12개월 범위 조회에 사용하고 review 매핑의 이름 경로 우회를 금지 |
| 1.2.5 | 2026-07-15 | 공용 고정 템플릿 렌더러와 `<단지명>_실거래추이_<최신확정월>.html` 동적 파일명 계약을 명시하고 일반 사용자 파일명 `result.html`·`index.html`을 금지 |
| 1.2.4 | 2026-07-15 | 사용자 노출 제목과 설명의 `시세추이`를 `실거래 추이`로 명확화. 국토교통부 신고 매매의 중위 실거래 추이이며 현재 가격 감정이 아님을 명시 |
| 1.0.0 | 2026-07-10 | 스킬 패키지화. 정적 데모 HTML을 `templates/result.html` 로 재작성 — `ipzi-data` JSON과 렌더 JS 신설(차트 좌표 데이터 구동, `esc()`·결측 처리, `ICONS` 화이트리스트). 디자인 규칙 인라인화, 폴더 밖 경로 제거, frontmatter 4키 정리 |
| 1.1.0 | 2026-07-10 | 🔴 **패키지화 때 떨어져 나간 차트 레이어 복원.** v1.0.0 은 데모의 대조선·신고지연 경계·계열색 평균선을 잃었고, 남은 평균선이 하필 `ipzitalk-complex-overview-all` 이 **대조선에 쓰는 `--x` `5 4`** 를 물려받아 같은 그림이 두 뜻이 됐다. 복원 내역: ① 대조선 `compareSeries`(선택) ② 신고지연 경계 + 주황 테두리 점 `provisionalFrom`(선택) ③ 기간 평균선을 계열색 `2 3` + 값 라벨로, 대조선 평균 `avgPyeongCompare` 병기 ④ 이동평균을 회색 → 계열색 35% ⑤ 결측 밴드에 `거래없음` 글자 ⑥ **확정창 고점·저점 라벨** 신설(신고지연·표본부족 달 제외). 선택 필드는 없으면 레이어를 생략하므로 기존 산출물은 그대로 유효 |
| 1.1.1 | 2026-07-10 | 🔴 **범례가 그리지 않은 레이어를 광고하던 버그 수정.** `provisionalFrom` 이 시계열 범위 밖이면 신고지연 점이 0개인데 범례에 항목이 떴다(템플릿 자신의 주석 위반). `provIdx` 를 함수 스코프로 올려 판정. 첫 달부터 신고지연이면 경계선 대신 좌상단 라벨로 알린다 |
| 1.2.3 | 2026-07-10 | 범례가 **그리지 않은 주황 점**을 광고하던 문제 수정(v1.1.1 과 같은 부류). 신고지연 달에 타입군 거래가 0건이면 점이 없는데 범례엔 떴다. 실제로 그린 점이 있을 때만 싣는다. 실측: 반포자이 26.06(대형 2건, 84㎡ 0건) |
| 1.2.2 | 2026-07-10 | 🔴 **`거래 없음`(n=0)에 `판정 유보` 태그가 붙던 회귀 수정.** v1.2.0 에서 태그를 `n` 에서 파생하게 바꾸며 `0 < 4` 가 함께 걸렸다. 거래가 드문 단지(방배롯데캐슬아르떼: 13개월 중 7개월 0건)에서 **13행 전부 같은 태그**가 되어 태그가 무의미해졌다. `신고지연` 은 0건이어도 유지한다("아직 안 들어왔다"는 뜻) |
| 1.2.1 | 2026-07-10 | v1.2.0 뒤처리. `ipzi-data` JSON 계약 블록에 `normalSampleFrom`·`extremesMinSample` 선언 추가(렌더 JS 는 읽는데 계약에는 없었다). 두 표본 문턱을 합치지 말라는 규칙과, 표본 얇은 단지(위례·반포자이)에서 극점 라벨이 사라지는 것이 의도임을 명시. 고점 사례로 인용하던 위례 26.04(N4)가 `n≥8` 규칙과 모순되어 헬리오시티 실측으로 교체. frontmatter version 이 이력보다 뒤처져 있던 것도 정정 |
| 1.2.0 | 2026-07-10 | 🔴 **KPI 앵커·고점 판정·태그 3건 교정.** ① 저점→고점 앵커(+12.4%)를 첫→최신 확정월(+3.9%)로 바꾸고 `고점 대비 -6.5%` 병기 ② 고점·저점 후보를 `n≥4` → `n≥8`(정상 표본)로 올림 — 4건이 7건을 0.15%로 이겨 고점이 뒤바뀌었다 ③ 표 태그를 `n`·`provisionalFrom` 에서 파생 — `판정 유보`+`신고지연` 이 함께 붙는다 |
