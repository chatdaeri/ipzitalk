...
각 공고는 사용자 첨부 파일이 없어도 공고명 또는 `house_manage_no`+`announcement_id`로 공식 원문을 확보한다.
...
6. 공식 경로가 모두 실패한 공고만 사용자에게 파일 첨부를 요청한다. 다른 공고의 원문이나 DB 값으로 대신하지 않는다.
...
각 공고의 공고명·관리번호·공고번호·공급위치·모집공고일을 원문과 대조한다. 충돌하면 값을 섞지 않는다. `audit.json.noticeResolutions[]`에 입력 방식, URL 출처, 후보·선택 파일, 선택 이유, 다운로드 횟수, hash, 일치 판정을 공고별로 남긴다.
