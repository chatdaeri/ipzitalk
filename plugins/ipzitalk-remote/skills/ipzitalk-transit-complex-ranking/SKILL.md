...
| `get_complex_info_batch(detail=true)` | (선택) 확정 코드의 K-apt 참고 도보시간 일괄 보강 |
| `get_map_embed_url` | 후보 지도 생성 |

**크레딧**: 역 좌표를 1콜로 모으고 거리 계산은 로컬이라 단지 수 `N`과 무관하다. 모든 MCP 호출을 센다.
```
구(舊):  keyword + enrich + get_complex_info(detail) × N   = 2 + N 크레딧
현재 기본(show_kapt_walk_min=false): 기준점 + keyword + enrich + category + 지도 = 5회
현재 참고열(show_kapt_walk_min=true): 기본 5회 + detail batch 1~2회 = 6~7회
```
...
3. **K-apt 매칭** — 외부 후보 배열을 `enrich_complex_info(complexes, radius_m=300~500)` 1회로 처리한다. `matched`만 랭킹 후보로 확정한다. `ambiguous`·`not_found`는 좌표·주소·사유만 표시하고 자동확정하지 않는다. `matched` 좌표는 재사용하고 추가 지오코딩하지 않으며, 좌표가 `null`인 확정 항목만 별도 지오코딩으로 해소한다.
...
7. **K-apt 참고 열 + 불일치 플래그** (`show_kapt_walk_min:true`일 때)
   - `matched` 코드만 모아 `get_complex_info_batch(detail=true)`를 최대 10건씩 1~2회 호출한다.
...
| 1.0.4 | 2026-08-12 | enrich `matched`만 랭킹 후보로 확정하고, `show_kapt_walk_min=true`일 때만 최대 10건씩 `detail=true` 배치 보강. 전체 MCP 호출 예산 5회 또는 6~7회 명시 |
